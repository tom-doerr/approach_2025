...
class TestInferLiveProbNames(unittest.TestCase):
    def test_prob_names_printed(self):
        import infer_live as inf
        tmp = tempfile.mkdtemp(); models_dir = os.path.join(tmp,'models'); os.makedirs(models_dir)
        try:
            from sklearn.linear_model import LogisticRegression
            import numpy as np
            clf = LogisticRegression(max_iter=5).fit(np.zeros((3,1)), np.array([0,1,2]))
            labels=["A","B","C"]
            ...
            assert any(t.startswith('prob_names=') for t in texts)
            line=[t for t in texts if t.startswith('prob_names=')][0]
