...
class TestInferLiveParamsPrint(unittest.TestCase):
    def test_prints_model_params_for_logreg(self):
        import infer_live as inf
        tmp = tempfile.mkdtemp(); models_dir = os.path.join(tmp, 'models'); os.makedirs(models_dir)
        try:
            from sklearn.linear_model import LogisticRegression
            import numpy as np
            clf = LogisticRegression(C=0.5, solver='lbfgs', max_iter=5).fit(np.zeros((3,1)), np.array([0,1,2]))
            ...
            out = buf.getvalue()
            assert 'Params:' in out and 'type=LogisticRegression' in out and 'C=' in out and 'solver=' in out
        finally:
