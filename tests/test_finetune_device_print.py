...
class TestFinetuneDevicePrint(unittest.TestCase):
    def test_prints_device_cpu(self):
        ...
        class FakeCap:
            def __init__(self, path):
                ...
                self.frames = [np.full((8, 8, 3), val, dtype=np.uint8) for _ in range(3)]
        ...
        try:
            args = SimpleNamespace(data="ignored", embed_model="mobilenetv3_small_100", backbone="mobilenetv3_small_100",
                                   ...
                                   clf="dl", epochs=1, batch_size=2, lr=1e-3, wd=0.0, device='cpu')
            tf.train(args)
        finally:
        ...
        out = buf.getvalue()
        assert "Device: cpu" in out or "Device: cpu".lower() in out.lower()
