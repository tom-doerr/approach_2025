import io, os, sys, shutil, tempfile, unittest
from types import SimpleNamespace


class TestEpochModelPathPrint(unittest.TestCase):
    def test_per_epoch_prints_plain_path_line(self):
        ...
        orig_list_videos_io = vio.list_videos; vio.list_videos = lambda _root: vids

        # Minimal cv2 stub
        import numpy as np
        class FakeCap:
            def __init__(self, path):
                val = 64 if path.endswith("A") else 192
        ...
        # Redirect saving to tmp models dir
        ...
        out = buf.getvalue()
        # Expect a plain path line containing /models/ and a .pkl filename
        lines = [ln.strip() for ln in out.splitlines() if "/models/" in ln and ln.strip().endswith(".pkl")]
        assert any("finetune_mobilenetv3_small_100_ep01" in ln for ln in lines)
