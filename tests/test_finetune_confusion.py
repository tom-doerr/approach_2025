...
class TestFinetuneConfusion(unittest.TestCase):
    def test_confusion_and_f1_print(self):
        ...
        class FakeCap:
            def __init__(self, path):
                ...
                self.frames = [np.full((10, 10, 3), val, dtype=np.uint8) for _ in range(12)]
            def isOpened(self): return True
            def read(self): return (True, self.frames.pop()) if self.frames else (False, None)
            def release(self): pass
            def get(self, prop): return 12
        ...
        class TinyNet:
            def __init__(self, num_classes=2):
                ...
                self.net = nn.Sequential(nn.Flatten(), nn.Linear(10*10*3, num_classes))
        ...
        try:
            args = SimpleNamespace(data="ignored", embed_model="mobilenetv3_small_100", backbone="mobilenetv3_small_100",
                                   eval_split=0.25, eval_mode='tail',
