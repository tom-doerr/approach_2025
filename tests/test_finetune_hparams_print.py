...
class TestFinetuneHParams(unittest.TestCase):
    def test_hparams_table_prints(self):
        ...
        class FakeCap:
            def __init__(self, path):
                val = 50 if path.endswith("A") else 200
        ...
        try:
            args = SimpleNamespace(data="ignored", embed_model="mobilenetv3_small_100", backbone="mobilenetv3_small_100",
                                   ...
                                   clf="dl", epochs=3, batch_size=7, lr=0.001, wd=0.01, device='cpu')
            tf.train(args)
        finally:
        ...
        out = buf.getvalue()
        assert "HParams" in out
        assert "epochs" in out and "3" in out
        assert "batch_size" in out and "7" in out
        assert "optimizer" in out and "AdamW" in out
