...
class TestFinetuneEpochSummary(unittest.TestCase):
    def test_epoch_summary_prints(self):
        ...
        class FakeCap:
            def __init__(self, path):
                val = 16 if path.endswith("A") else 200
                self.frames = [np.full((12, 12, 3), val, dtype=np.uint8) for _ in range(8)]
        ...
        class TinyNet:
            def __init__(self, num_classes=2):
                ...
                self.net = nn.Sequential(nn.Flatten(), nn.Linear(12*12*3, num_classes))
        ...
        try:
            args = SimpleNamespace(data="ignored", embed_model="mobilenetv3_small_100", backbone="mobilenetv3_small_100",
                                   ...
                                   clf="dl", epochs=2, batch_size=4, lr=1e-3, wd=0.0, device='cpu')
            tf.train(args)
        finally:
