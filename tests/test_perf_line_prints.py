...
class TestPerfLine(unittest.TestCase):
    def test_perf_line_includes_bs_and_throughput(self):
        ...
        class FakeCap:
            def __init__(self, path):
                val = 10 if path.endswith("A") else 20
        ...
        class TinyNet:
            def __init__(self, num_classes=2):
                ...
                # Adapt to any HxW via GAP → 3 features
                self.net = nn.Sequential(nn.AdaptiveAvgPool2d((1,1)), nn.Flatten(), nn.Linear(3, num_classes))
        ...
        out = buf.getvalue()
        # Ensure Perf appears and includes batch size; throughput may wrap
        assert "Perf:" in out
        perf_lines = [ln for ln in out.splitlines() if "Perf:" in ln]
        pl = " ".join(perf_lines)
        assert " bs=" in pl
