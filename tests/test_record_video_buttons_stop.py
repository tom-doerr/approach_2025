...
class TestRecordVideoButtonsStop(unittest.TestCase):
    def test_stop_button_finishes_recording(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                clicks = 0
                def waitKey_click_label_then_stop(*a, **k):
                    nonlocal clicks
                    ...
                    if clicks == 0:
                        ...
                        # click STOP
                        lab, x0, y0, x1, y1 = boxes[-1]
                        cv._cb(cv.EVENT_LBUTTONDOWN, x0+2, y0+2, 0, None)
                    clicks += 1
                    if clicks < 3:
                        return 0  # keep looping once more
                    return orig_wait(*a, **k)
                cv.waitKey = waitKey_click_label_then_stop
                record_buttons("A,B", cam_index=0)
                self.assertIsNotNone(events["path"])  # writer created
                self.assertGreaterEqual(events["released"], 1)  # stop released writer
                os.chdir(cwd)
        finally:
