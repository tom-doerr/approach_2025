...
def parse_args(argv=None):
    ...
    p.add_argument("--mp-stride", type=int, default=1, help="MP: process every Nth frame per video (1 = no stride, default)")
...
def _hpo_logreg(X, y, iters: int, seed: int = 0, logger=None, idx_by_class=None, eval_frac: float | None = None, max_iter: int = 500, solver: str = 'lbfgs'):
    ...
    trials = []
    for i in range(max(1, iters)):
        # Wider default search: 1e-4 .. 1e6
        C = float(10 ** rng.uniform(-4, 6))
